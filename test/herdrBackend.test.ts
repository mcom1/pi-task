...
test("HerdR launch returns a socket-scoped terminal handle", async () => {
  ...
  assert.deepEqual(calls[0]?.args.slice(0, 3), ["agent", "start", "pi-task"]);
  assert.deepEqual(calls[0]?.args.slice(-4), ["--", "sh", "-lc", "pi --session task"]);
...
test("sync steering accepts HerdR mutation commands with empty stdout", () => {
  ...
  assert.deepEqual(calls.at(-2), ["pane", "run", "w1:p2", "follow up"]);
  assert.deepEqual(calls.at(-1), ["pane", "send-keys", "w1:p2", "enter"]);
});
